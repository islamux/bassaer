...
export function rotateAgentLog(log) {
...
export function getArchivedLogCount() {
    ensureArchiveDir();
    const files = fs.readdirSync(ARCHIVE_DIR).filter(f => f.startsWith('agent-log-') && f.endsWith('.json'));
    let total = 0;
    for (const f of files) {
        try {
            const entries = JSON.parse(fs.readFileSync(path.join(ARCHIVE_DIR, f), 'utf-8'));
