import { readTracker, writeTracker, findTask, touchAgent, pushLog, autoUnblockDependents, countRevisions, ok, fail, } from './tracker.service.js';
export function startTask(taskId, agentId = 'orchestrator') {
...
export function completeTask(taskId, summary, agentId) {
    const state = readTracker();
    const found = findTask(state, taskId);
    if (!found)
        return fail(`Task not found: ${taskId}`);
...
export function approveTask(taskId, feedback) {
    const state = readTracker();
    const found = findTask(state, taskId);
    if (!found)
        return fail(`Task not found: ${taskId}`);
...
export function rejectTask(taskId, feedback) {
    const state = readTracker();
    const found = findTask(state, taskId);
    if (!found)
        return fail(`Task not found: ${taskId}`);
...
export function unblockTask(taskId, resolution) {
    const state = readTracker();
    const found = findTask(state, taskId);
    if (!found)
        return fail(`Task not found: ${taskId}`);
...
export function updateTask(taskId, updates) {
    ...
    if (updates.priority !== undefined) {
        subtask.priority = updates.priority;
        changes.push(`priority=${updates.priority}`);
    }
    if (updates.assignee !== undefined) {
        subtask.assignee = updates.assignee;
        changes.push(`assignee=${updates.assignee}`);
    }
    if (updates.execution_mode !== undefined) {
        subtask.execution_mode = updates.execution_mode;
        changes.push(`execution_mode=${updates.execution_mode}`);
    }
...
export function logAction(taskId, action, description, agentId = 'orchestrator', tags = []) {
    const state = readTracker();
    touchAgent(state, agentId);
    pushLog(state, {
        agent_id: agentId,
...
export function enrichTask(taskId, updates) {
    const state = readTracker();
    const found = findTask(state, taskId);
    if (!found)
        return fail(`Task not found: ${taskId}`);
    ...
    if (updates.prompt !== undefined) {
        subtask.prompt = updates.prompt;
        changes.push('prompt');
    }
    if (updates.builder_prompt !== undefined) {
        subtask.builder_prompt = updates.builder_prompt;
        changes.push('builder_prompt');
    }
    if (updates.acceptance_criteria !== undefined) {
        subtask.acceptance_criteria = updates.acceptance_criteria;
        changes.push('acceptance_criteria');
    }
    if (updates.constraints !== undefined) {
        subtask.constraints = updates.constraints;
        changes.push('constraints');
    }
    if (updates.context_files !== undefined) {
        subtask.context_files = updates.context_files;
        changes.push('context_files');
    }
    if (updates.reference_docs !== undefined) {
        subtask.reference_docs = updates.reference_docs;
