import {
  ...
  type TrackerState, type Subtask, type Milestone
} from './tracker.js'
import {
...
function getMilestoneById(state: TrackerState, milestoneId: string): Milestone | undefined {
...
function toolGetTaskContext(args: Record<string, any>): ToolResult {
...
function toolGetTaskSummary(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const found = findTask(state, args.task_id)
  if (!found) return err(`Task not found: ${args.task_id}`)
...
function toolGetProjectStatus(): ToolResult {
  const state = readTracker()
  return ok(buildProjectStatus(state))
}

function toolGetMilestoneOverview(args: Record<string, any>): ToolResult {
  const state = readTracker()
...
function toolGetTaskHistory(args: Record<string, any>): ToolResult {
  const state = readTracker()
  ...
  const lines: string[] = [`# History for ${args.task_id}`, '']
...
function toolListAgents(): ToolResult {
  const state = readTracker()
...
function toolStartTask(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const found = findTask(state, args.task_id)
  if (!found) return err(`Task not found: ${args.task_id}`)
...
function toolCompleteTask(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const found = findTask(state, args.task_id)
  if (!found) return err(`Task not found: ${args.task_id}`)
...
function toolApproveTask(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const found = findTask(state, args.task_id)
  if (!found) return err(`Task not found: ${args.task_id}`)
...
function toolRejectTask(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const found = findTask(state, args.task_id)
  if (!found) return err(`Task not found: ${args.task_id}`)
...
function toolResetTask(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const found = findTask(state, args.task_id)
  if (!found) return err(`Task not found: ${args.task_id}`)
...
function toolBlockTask(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const found = findTask(state, args.task_id)
  if (!found) return err(`Task not found: ${args.task_id}`)
...
function toolUnblockTask(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const found = findTask(state, args.task_id)
  if (!found) return err(`Task not found: ${args.task_id}`)
...
function toolUpdateTask(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const found = findTask(state, args.task_id)
  if (!found) return err(`Task not found: ${args.task_id}`)
  ...
  if (args.priority !== undefined) { subtask.priority = args.priority; changes.push(`priority=${args.priority}`) }
  if (args.assignee !== undefined) { subtask.assignee = args.assignee; changes.push(`assignee=${args.assignee}`) }
  if (args.execution_mode !== undefined) { subtask.execution_mode = args.execution_mode; changes.push(`execution_mode=${args.execution_mode}`) }
  if (args.notes !== undefined) { subtask.notes = args.notes; changes.push('notes updated') }
...
function toolLogAction(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const agentId = args.agent_id || 'orchestrator'
...
function toolEnrichTask(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const found = findTask(state, args.task_id)
  if (!found) return err(`Task not found: ${args.task_id}`)
  ...
  if (args.prompt !== undefined) { subtask.prompt = args.prompt; changes.push('prompt') }
  if (args.builder_prompt !== undefined) { subtask.builder_prompt = args.builder_prompt; changes.push('builder_prompt') }
  if (args.acceptance_criteria !== undefined) { subtask.acceptance_criteria = args.acceptance_criteria; changes.push('acceptance_criteria') }
  if (args.constraints !== undefined) { subtask.constraints = args.constraints; changes.push('constraints') }
  if (args.context_files !== undefined) { subtask.context_files = args.context_files; changes.push('context_files') }
  if (args.reference_docs !== undefined) { subtask.reference_docs = args.reference_docs; changes.push('reference_docs') }
...
function toolAddMilestoneNote(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const milestone = getMilestoneById(state, args.milestone_id)
  if (!milestone) return err(`Milestone not found: ${args.milestone_id}`)
...
function toolSetMilestoneDates(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const milestone = getMilestoneById(state, args.milestone_id)
  if (!milestone) return err(`Milestone not found: ${args.milestone_id}`)
...
function toolUpdateDrift(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const milestone = getMilestoneById(state, args.milestone_id)
  if (!milestone) return err(`Milestone not found: ${args.milestone_id}`)
...
function toolCreateMilestone(args: Record<string, any>): ToolResult {
  const state = readTracker()
...
function toolAddMilestoneTask(args: Record<string, any>): ToolResult {
  const state = readTracker()
  const milestone = getMilestoneById(state, args.milestone_id)
  if (!milestone) return err(`Milestone not found: ${args.milestone_id}`)
...
function toolRegisterAgent(args: Record<string, any>): ToolResult {
  const state = readTracker()
  ...
  if (existing) {
    ...
    if (args.color !== undefined) existing.color = args.color
    if (args.parent_id !== undefined) existing.parent_id = args.parent_id
  } else {
    state.agents.push({
      id: agentId,
      name: args.name,
