...
| **C3** | **`migrateTracker` is a No-Op** | `migration.service.ts` | **FIXED** — `runMigrations()` is now called in `tracker.service.ts:readTracker()` (line 10). Schema validation + migration now works. |
