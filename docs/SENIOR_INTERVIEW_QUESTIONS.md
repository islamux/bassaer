...
### Q64. The `contentLoader.test.ts` mocks `fs`. What's the subtle gap it leaves?

**A:** `lib/__tests__/contentLoader.test.ts` mocks **only** `fs` (`vi.mock("fs", ...)` at `:3`, supplying `existsSync`/`readdirSync`/`readFileSync`). `path` is **not** mocked — it stays the real module — and the fs-mock's keys are derived from the real `process.cwd()` (`${process.cwd()}/content/chapters/...`, `:4-9`), so the test does exercise real cwd-based path resolution. The subtle gap is **fs-level**, not path-level: the test runs under jsdom against a mocked `fs`, so the actual reading of the real `content/chapters/*.md` files from disk is never exercised — `readFileSync` returns canned strings, bypassing real I/O. So the mock verifies the parsing/ordering logic against fixture content, but not integration with the real files on disk (real `readFileSync`/`readdirSync`, real `path.resolve(process.cwd(), 'content', 'chapters')`, `lib/contentLoader.ts:4`). A real-fs integration check (a fixtures dir, or reading the actual `content/chapters`) would close that gap.
